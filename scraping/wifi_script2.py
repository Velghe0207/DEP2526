...
def main():
    ...
    # pad naar de map
    output_dir = "scraping/backup_wifi"
    os.makedirs(output_dir, exist_ok=True)  # zorgt dat de folder bestaat

    # Generate timestamp for filename (Brussels time)
    tz = pytz.timezone("Europe/Brussels")
    now_brussels = datetime.now(tz).strftime("%Y%m%d_%H%M%S")

    # Build filename with timestamp
    output_file = os.path.join(output_dir, f"backup_wifi_{now_brussels}.csv")

    # Write CSV
    df_wifi_edu.to_csv(output_file, index=False)
